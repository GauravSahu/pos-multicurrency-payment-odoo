...
class pos_order(osv.osv):
    ...
    def _amount_all(self, cr, uid, ids, name, args, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for payment in order.statement_ids:
                res[order.id]['amount_paid'] +=  payment.paid_amount
                print "-------------------->>>",payment.paid_amount
                res[order.id]['amount_return'] += (payment.paid_amount < 0 and payment.paid_amount or 0)
    ...
    def add_payment(self, cr, uid, order_id, data, context=None):
        ...
        if journal_id: 
            curr_id = self.pool.get('account.journal').browse(cr,uid,journal_id).currency.id
            company_id = self.pool.get('pos.order').browse(cr,uid,order_id).company_id.id
            if curr_id==False:
                curr_id = self.pool.get('res.company').browse(cr,uid,company_id).currency_id.id
            args['paid_amount'] = args['amount']/self.pool.get('res.currency').browse(cr,uid,curr_id).rate_silent
