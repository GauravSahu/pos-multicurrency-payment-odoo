function openerp_pos_basewidget(instance, module){ //module is instance.point_of_sale
    ...
    module.PosBaseWidget = instance.web.Widget.extend({
		...
		currency_gaurav = this.pos.currency;
            }else{
            ...
            this.format_currency = function(amount,a){
                if(typeof amount === 'number'){
                    amount = Math.round(amount*100)/100;
                    amount = amount.toFixed(decimals);
                }
                if(this.currency.position === 'after'){
                    var b = this.pos.all_currency;
                    var curr_id = a;
                    _.each(b, function(curr){
                        if(curr.id === curr_id){
                            currency_gaurav =  curr;
                           
                        }
                    })
                   
                    return amount + ' ' + currency_gaurav.symbol;
                }else{
                    return currency_gaurav.symbol + ' ' + amount;
                }
            }
